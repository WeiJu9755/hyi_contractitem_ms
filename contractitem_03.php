...
$now = date("Y-m-d");
$annual_day = isset($_GET['annual_day']) ? $_GET['annual_day'] : $now;
...
$get_contract_dropdown = isset($_GET['contract_id']) ? $_GET['contract_id'] : '';
...
if ($mDB->rowCount() > 0) {
	while ($row = $mDB->fetchRow(2)) {
		$select_contract_id   = $row['contract_id'];
    $select_contract_caption  = $row['contract_caption'];
		$selected = ($get_contract_dropdown === $select_contract_id) ? ' selected' : '';
		$select_contract .= "<option value='$select_contract_id' $selected>$select_contract_caption</option>";
	}
}
...
$Qry = "SELECT contract_id,contract_caption FROM contract
  WHERE contract_id = '$get_contract_dropdown'
  ";
...
$casereport_list .= <<<EOT
<div class="m-auto" style="width:80%;min-height:300px;margin-bottom:100px;">
	<div class="w-100">
...
EOT;
			...
			$casereport_list .= <<<EOT
				<tr>
					<td colspan="11" class="text-center p-3">無任何符合查詢的資料222</td>
				</tr>
EOT;
		} 
...
EOT;
	...
	$casereport_list .= <<<EOT
	<div class="size16 weight p-5 text-center">無任何符合查詢的資料123</div>
EOT;
}
...
<div class="container-fluid p-3 text-center bg-white mt-3">
  <h3 class="mb-4">合約工項日報表</h3>

 <div class="d-flex justify-content-center align-items-center flex-wrap gap-3">

  <!-- 合約 -->
  <div class="text-center">
    <label class="form-label fw-bold mb-0 me-2">合約：</label>
    <span>$select_contract</span>
  </div>

  <!-- 日期 -->
  <div class="d-flex align-items-center">
    <label class="form-label fw-bold mb-0 me-2">日期：</label>
  ...
  <!-- 查詢按鈕 -->
  <div>
    <button type="button" class="btn btn-success" onclick="chdatetime();">
...
function chdatetime() {
  ...
  var newUrl = '?ch=contractitem_03&fm=contractitem'
